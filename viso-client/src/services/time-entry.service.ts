...
class TimeEntryService {
  ...
  async getAll() {
    const { data } = await appApi.get<TTimeEntry>(API_URL.timeEntry());
    return data;
  }

  async create(data: TTimeEntryInput) {
    const { data: response } = await appApi<TTimeEntry>({url: API_URL.timeEntry(), method: 'POST', data});
    return response;
  }

  async update(data: TTimeEntryUpdate) {
    const { data: response } = await appApi<TTimeEntry>({url: API_URL.timeEntry(), method: 'PATCH', data});
    return response;
  }
}
